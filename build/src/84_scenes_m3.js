/* ==========================================================================
   EE311 · Module 3 — Linear Time-Invariant Systems  [Source: 14–21]
   ========================================================================== */
(function(){
...
{ id:'m3-steps', module:'M3', nav:'Flip, shift, multiply, add', title:'The four mechanical steps', src:'p. 15',
  ...
  {t:'cols', ratio:'c-5-7', vcenter:true, left:[
    ...
    {t:'reveal', at:1, items:[
      {t:'note', kind:'warn', head:'Build $h[n-k]$ the same way you built $x(at-b)$', html:'Treat $k$ as the variable: $h[n-k]=h\\bigl(-(k-n)\\bigr)$. Shift $h[k]$ by $n$, then reverse in $k$ — the “shift, then scale” routine of Module 1 with scale factor $-1$. '}]},
    {t:'reveal', at:2, items:[
...
{ id:'m3-ex-dt1', module:'M3', nav:'Worked example · finite sequences', title:'Worked example — two finite sequences', src:'pp. 15–16',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'reveal', at:3, items:[
      {t:'note', kind:'def', head:'Exercise', html:'Verify the same result using the equivalent form $y[n]=\\sum_k h[k]x[n-k]$ — that is, flip $x$ instead of $h$. Commutativity guarantees the same answer and the check is worth doing once by hand.'}]}
  ], right:[
...
{ id:'m3-convint', module:'M3', nav:'The convolution integral', title:'The convolution integral', src:'pp. 17–18',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'reveal', at:2, items:[
      {t:'note', kind:'warn', head:'Building $h(t-\\tau)$ correctly', html:'Build it by shift, then scale, in the variable $	au$. Example: to plot $\\delta(-t+5)$, first $v(t)=\\delta(t+5)$, then $y(t)=v(-t)=\\delta(-t+5)$ — an impulse at $t=5$. The same routine builds $h(t-\\tau)$, and it is where sign errors are made if it is done by eye.'}]}
  ], right:[
    {t:'fig', frame:true, svg:()=>{
    ...
    {t:'reveal', at:1, items:[
      {t:'fig', frame:true, svg:()=>{
        ...
        return a.svg(); },
        caption:'The continuous-time representation property, illustrated. Each impulse carries weight $x(\\tau)\\d\\tau$; the system responds to each with $h(t-\\tau)$ and the integral superposes them.'}]}
  ]}
]},
...
{ id:'m3-ex-ct1', module:'M3', nav:'Worked example · CT case split', title:'Worked example — a two-case convolution', src:'pp. 18–19',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'grid', cols:1, gap:'6px', items:[
      ...
      [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:180,xr:[-6,8],yr:[-0.1,1.2],xlabel:'t',pad:{l:48,r:22,t:14,b:28},xtarget:8,ytarget:2});
        a.curve(t=>t>=3?1:0,{color:C.h}); a.note(2.7,0.55,'h(t) = u(t−3)',{anchor:'end',color:C.h,fs:14,italic:true}); return a.svg(); }}]
    ]},
    {t:'reveal', at:1, items:[
      {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:180,xr:[-6,8],yr:[-0.1,1.25],xlabel:'\\tau',pad:{l:46,r:20,t:14,b:28},xtarget:8,ytarget:2});
        ...
        a.note(t-3-0.18,1.14,'τ = t − 3',{anchor:'end',color:C.coral,fs:13});
...
{ id:'m3-ex-ct2', module:'M3', nav:'Worked example · five cases', title:'Worked example — a five-case convolution', src:'pp. 19–20',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'reveal', at:4, items:[
      ...
      {t:'instr', head:'Exercise', html:'Repeat with $x(t)=1$ on $0<t<3$ and the same $h$. The window is now <em>wider</em> than the ramp, which changes the middle case — a test of whether the boundary list was understood rather than memorised.'}]}
  ], right:[
    {t:'grid', cols:2, gap:'12px', items:[
    ...
    {t:'reveal', at:3, items:[
      ...
      {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:180,xr:[-0.5,3.5],yr:[-0.15,1.75],xlabel:'t',pad:{l:48,r:20,t:14,b:28},xtarget:5,ytarget:3});
        ...
        return a.svg(); },
        caption:'Peak $1.5$ at $t=2$, matching the definition MATLAB figure.'}]}
  ]}
]},
