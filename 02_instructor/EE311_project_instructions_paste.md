...
**Before doing anything, read `CLAUDE.md` in the project root**, then `02_instructor/PHASE2_HANDOFF.md`
and `02_instructor/EE311_STYLE_RULES.md`. The design system, build pipeline and content schema are
...
- Rebuild only from `03_production/EE311_Deliverables.zip`. The zip in `_archive/` is stale and
...
- Do the work in the container, deliver with `SendUserFile`, then write back into the numbered folders
  (`00_source` / `01_student` / `02_instructor` / `03_production`, superseded material to `_archive`).
