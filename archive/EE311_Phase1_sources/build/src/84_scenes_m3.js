/* ==========================================================================
   EE311 · Module 3 — Linear Time-Invariant Systems  [Source: PDF pp. 14–21]
   ========================================================================== */
(function(){
...
{ id:'m3-open', module:'M3', nav:'Module 3 opening', title:'Linear Time-Invariant Systems', src:'pp. 14–21',
  ...
  {t:'lede', text:'Impose linearity and time invariance together, and an entire system — every response to every possible input — is encoded in its response to a single impulse.'},
...
{ id:'m3-impulse', module:'M3', nav:'Impulse response', title:'The impulse response', src:'p. 14',
  ...
  {t:'cols', ratio:'c-5-7', vcenter:true, left:[
    {t:'note', kind:'def', head:'Definition (source)', html:'The <b>impulse response</b> is the response of the system to a unit impulse:<br>$x[n]=\\delta[n]\\;\\to\\;S\\;\\to\\;y[n]={{sym:ht|h[n]}}$.'},
    {t:'reveal', at:1, items:[
      {t:'body', html:'For an arbitrary system this is one data point among infinitely many — knowing the response to $\\delta[n]$ tells you nothing about the response to anything else.'},
    ...
    {t:'reveal', at:2, items:[
      {t:'note', kind:'warn', head:'A practical caveat worth stating', html:'Real impulses do not exist. In practice $h$ is measured with a short pulse, a step (then differentiated), or a broadband sequence — all justified by the same linearity that makes $h$ meaningful in the first place.'}]}
  ], right:[
    {t:'fig', frame:true, svg:()=>P.blocks({w:820,h:220,items:[
      ...
      {t:'text',x:195,y:92,label:'x[n] = δ[n]',italic:true,fs:17,color:'#14707F'},
      {t:'text',x:195,y:132,label:'unit impulse',fs:12},
      {t:'text',x:600,y:92,label:'y[n] = h[n]',italic:true,fs:17,color:'#C08422'},
      {t:'text',x:600,y:132,label:'impulse response',fs:12}
    ]}), caption:'Redrawn from the source block diagram (PDF p. 14).'},
    {t:'reveal', at:1, items:[
      {t:'grid', cols:2, gap:'20px', items:[
        [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:520,h:220,xr:[-2,7],yr:[-0.3,1.35],xlabel:'n',pad:{l:46,r:22,t:18,b:32},xtarget:6,ytarget:2});
          a.stem(disc(n=>n===0?1:0,-2,7),{color:C.in}); a.note(6.6,1.16,'δ[n]',{anchor:'end',color:C.in,fs:15,italic:true}); return a.svg(); }}],
        [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:520,h:220,xr:[-2,7],yr:[-0.3,1.35],xlabel:'n',pad:{l:46,r:22,t:18,b:32},xtarget:6,ytarget:2});
          a.stem(disc(n=>n>=0?Math.pow(0.72,n):0,-2,7),{color:C.h}); a.note(6.6,1.16,'h[n]',{anchor:'end',color:C.h,fs:15,italic:true}); return a.svg(); }}]
      ]}]}
  ]}
]},
...
{ id:'m3-representation', module:'M3', nav:'Representation property', title:'Every signal is a sum of impulses', src:'pp. 14–15',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'eq', size:'sm', tex:'\\begin{aligned} x[n]\\delta[n+1]&=x[-1]\\delta[n+1]\\\\ x[n]\\delta[n]&=x[0]\\delta[n]\\\\ x[n]\\delta[n-1]&=x[1]\\delta[n-1]\\\\ x[n]\\delta[n-2]&=x[2]\\delta[n-2] \\end{aligned}',
      label:'Sampling, applied at every $n_0$'},
    {t:'reveal', at:1, items:[
    ...
    {t:'reveal', at:3, items:[
      {t:'note', kind:'def', head:'Read the two indices carefully', html:'$n$ is the free time variable of the resulting signal; $k$ is a dummy summation index that runs over all shifts. Confusing them is the single most common source of sign and limit errors in the convolution sum that follows.'}]}
  ], right:[
    {t:'fig', frame:true, svg:()=>{
      ...
      a.stem(disc(x,-2,6),{color:C.in}); a.note(5.6,2.25,'x[n]',{anchor:'end',color:C.in,fs:15,italic:true});
      return a.svg(); }},
    {t:'reveal', at:2, items:[
      {t:'grid', cols:2, gap:'12px', items:[
        [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:420,h:180,xr:[-2,6],yr:[-0.3,2.5],xlabel:'n',pad:{l:42,r:18,t:16,b:30},xtarget:5,ytarget:2});
          a.stem(disc(n=>n===0?1:0,-2,6),{color:C.mid}); a.note(5.6,2.1,'1·δ[n]',{anchor:'end',color:C.mid,fs:13}); return a.svg(); }}],
        [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:420,h:180,xr:[-2,6],yr:[-0.3,2.5],xlabel:'n',pad:{l:42,r:18,t:16,b:30},xtarget:5,ytarget:2});
          a.stem(disc(n=>n===1?2:0,-2,6),{color:C.mid}); a.note(5.6,2.1,'2·δ[n−1]',{anchor:'end',color:C.mid,fs:13}); return a.svg(); }}],
        [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:420,h:180,xr:[-2,6],yr:[-0.3,2.5],xlabel:'n',pad:{l:42,r:18,t:16,b:30},xtarget:5,ytarget:2});
          a.stem(disc(n=>n===2?1:0,-2,6),{color:C.mid}); a.note(5.6,2.1,'1·δ[n−2]',{anchor:'end',color:C.mid,fs:13}); return a.svg(); }}],
        [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:420,h:180,xr:[-2,6],yr:[-0.3,2.5],xlabel:'n',pad:{l:42,r:18,t:16,b:30},xtarget:5,ytarget:2});
          a.stem(disc(n=>n===3?2:0,-2,6),{color:C.mid}); a.note(5.6,2.1,'2·δ[n−3]',{anchor:'end',color:C.mid,fs:13}); return a.svg(); }}]
      ]},
      {t:'small', html:'The four components sum, sample by sample, to the signal above. <b>Editorial illustration</b> of the source identity.'}]}
  ]}
]},
...
{ id:'m3-convsum', module:'M3', nav:'The convolution sum', title:'Deriving the convolution sum', src:'p. 15',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'reveal', at:1, items:[
      {t:'note', kind:'ok', head:'Use time invariance', html:'$\\delta[n]\\to h[n]$, therefore $\\delta[n-k]\\to h[n-k]$ — the shifted impulse produces the shifted impulse response.'}]},
    {t:'reveal', at:2, items:[
    ...
    {t:'reveal', at:4, items:[
      ...
      {t:'note', kind:'err', head:'The precondition, restated', html:'Convolution can only be applied when the system is <b>LTI</b>. The derivation used time invariance once and linearity twice; remove either and no step survives.'}]}
  ], right:[
    {t:'fig', frame:true, svg:()=>P.blocks({w:820,h:400,items:[
      {t:'text',x:60,y:36,label:'δ[n]',anchor:'start',italic:true,fs:16,color:'#14707F'},
      {t:'arrow',x1:130,y1:30,x2:300,y2:30},{t:'box',x:300,y:8,w:110,h:44,label:'S'},
      {t:'arrow',x1:410,y1:30,x2:560,y2:30},
      {t:'text',x:620,y:36,label:'h[n]',anchor:'start',italic:true,fs:16,color:'#C08422'},
      {t:'text',x:60,y:126,label:'δ[n−k]',anchor:'start',italic:true,fs:16,color:'#14707F'},
      {t:'arrow',x1:160,y1:120,x2:300,y2:120},{t:'box',x:300,y:98,w:110,h:44,label:'S'},
      {t:'arrow',x1:410,y1:120,x2:560,y2:120},
      {t:'text',x:620,y:126,label:'h[n−k]',anchor:'start',italic:true,fs:16,color:'#C08422'},
      {t:'text',x:355,y:88,label:'time invariance',fs:12,color:'#4A657F'},
      {t:'text',x:60,y:226,label:'x[k]·δ[n−k]',anchor:'start',italic:true,fs:16,color:'#14707F'},
      {t:'arrow',x1:200,y1:220,x2:300,y2:220},{t:'box',x:300,y:198,w:110,h:44,label:'S'},
      {t:'arrow',x1:410,y1:220,x2:560,y2:220},
      {t:'text',x:620,y:226,label:'x[k]·h[n−k]',anchor:'start',italic:true,fs:16,color:'#C08422'},
      {t:'text',x:355,y:188,label:'homogeneity',fs:12,color:'#4A657F'},
      {t:'text',x:60,y:336,label:'Σₖ x[k]δ[n−k]',anchor:'start',italic:true,fs:16,color:'#14707F'},
      {t:'arrow',x1:220,y1:330,x2:300,y2:330},{t:'box',x:300,y:308,w:110,h:44,label:'S'},
      {t:'arrow',x1:410,y1:330,x2:560,y2:330},
      {t:'text',x:620,y:336,label:'Σₖ x[k]h[n−k]',anchor:'start',italic:true,fs:16,color:'#4A7A46'},
...
{ id:'m3-steps', module:'M3', nav:'Flip, shift, multiply, add', title:'The four mechanical steps', src:'p. 15',
  ...
  {t:'cols', ratio:'c-5-7', vcenter:true, left:[
    {t:'note', kind:'def', head:'Steps of convolution (source)', html:'<b>(1) Flip.</b> Reverse $h[k]$ to obtain $h[-k]$.<br><b>(2) Shift.</b> Displace it by $n$ to obtain $h[n-k]$.<br><b>(3) Multiply and add.</b> Form $x[k]h[n-k]$ and sum over $k$.<br>Repeat for every $n$.'},
    {t:'reveal', at:1, items:[
      {t:'note', kind:'warn', head:'Build $h[n-k]$ the same way you built $x(at-b)$', html:'Treat $k$ as the variable: $h[n-k]=h\\bigl(-(k-n)\\bigr)$. Shift $h[k]$ by $n$, then reverse in $k$ — the “shift, then scale” routine of Module 1 with scale factor $-1$. The source does exactly this.'}]},
    {t:'reveal', at:2, items:[
      {t:'note', kind:'err', head:'What happens if you skip the flip', html:'Sliding an unflipped $h$ computes $\\sum_k x[k]h[k-n]$ — the <b>cross-correlation</b> of $x$ and $h$, not the convolution. For symmetric $h$ the two agree, which is precisely why the error survives undetected until an asymmetric $h$ appears in an exam.'}]}
  ], right:[
    {t:'fig', frame:true, svg:()=>{
      ...
      a.stem(disc(h,-5,7),{color:C.h}); a.note(6.6,1.14,'h[k]',{anchor:'end',color:C.h,fs:15,italic:true}); return a.svg(); },
      caption:'<b>Given.</b> An asymmetric impulse response — the only kind that exposes a missing flip.'},
    {t:'fig', frame:true, svg:()=>{
      const h=k=>(k>=0&&k<=2)?[1,0.6,0.3][k]:0;
      const a=P.Axes({w:820,h:200,xr:[-5,7],yr:[-0.25,1.3],xlabel:'k',pad:{l:48,r:24,t:18,b:32},xtarget:9,ytarget:2});
      a.stem(disc(k=>h(-k),-5,7),{color:C.mid}); a.note(6.6,1.14,'h[−k]  — step 1, flip',{anchor:'end',color:C.mid,fs:15,italic:true}); return a.svg(); }},
    {t:'fig', frame:true, svg:()=>{
      const h=k=>(k>=0&&k<=2)?[1,0.6,0.3][k]:0;
      const a=P.Axes({w:820,h:200,xr:[-5,7],yr:[-0.25,1.3],xlabel:'k',pad:{l:48,r:24,t:18,b:32},xtarget:9,ytarget:2});
      a.stem(disc(k=>h(3-k),-5,7),{color:C.out}); a.vline(3,{color:C.coral});
      a.note(6.6,1.14,'h[n−k] at n = 3  — step 2, shift',{anchor:'end',color:C.out,fs:15,italic:true}); return a.svg(); },
      caption:'The reversed response is anchored at $k=n$: its <em>first</em> sample now sits at the current instant and its tail reaches back into the past. That is causality, drawn.'}
  ]}
]},

{ id:'m3-ex-dt1', module:'M3', nav:'Worked example · finite sequences', title:'Worked example — two finite sequences', src:'pp. 15–16',
  objective:'Reproduce the source example with the superposition method and verify.',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'reveal', at:3, items:[
      {t:'note', kind:'def', head:'Homework from the source', html:'Verify the same result using the equivalent form $y[n]=\\sum_k h[k]x[n-k]$ — that is, flip $x$ instead of $h$. Commutativity guarantees the same answer and the check is worth doing once by hand.'}]}
  ], right:[
    {t:'grid', cols:1, gap:'6px', items:[
      [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:170,xr:[-1,6],yr:[-0.3,2.4],xlabel:'n',pad:{l:46,r:22,t:14,b:28},xtarget:7,ytarget:2});
        a.stem(disc(n=>(n>=0&&n<=3)?[1,2,1,2][n]:0,-1,6),{color:C.in}); a.note(5.6,2.1,'x[n]',{anchor:'end',color:C.in,fs:14,italic:true}); return a.svg(); }}],
      [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:170,xr:[-1,6],yr:[-0.3,2.4],xlabel:'n',pad:{l:46,r:22,t:14,b:28},xtarget:7,ytarget:2});
        a.stem(disc(n=>(n>=0&&n<=1)?1:0,-1,6),{color:C.h}); a.note(5.6,2.1,'h[n]',{anchor:'end',color:C.h,fs:14,italic:true}); return a.svg(); }}]
    ]},
    {t:'reveal', at:1, items:[
      {t:'grid', cols:2, gap:'8px', items:[
        [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:410,h:150,xr:[-1,6],yr:[-0.3,2.4],xlabel:'n',pad:{l:40,r:16,t:12,b:26},xtarget:4,ytarget:2});
          a.stem(disc(n=>(n>=0&&n<=1)?1:0,-1,6),{color:C.mid,r:3}); a.note(5.6,2.0,'h[n]',{anchor:'end',color:C.mid,fs:12}); return a.svg(); }}],
        [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:410,h:150,xr:[-1,6],yr:[-0.3,2.4],xlabel:'n',pad:{l:40,r:16,t:12,b:26},xtarget:4,ytarget:2});
          a.stem(disc(n=>(n>=1&&n<=2)?2:0,-1,6),{color:C.mid,r:3}); a.note(5.6,2.0,'2h[n−1]',{anchor:'end',color:C.mid,fs:12}); return a.svg(); }}],
        [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:410,h:150,xr:[-1,6],yr:[-0.3,2.4],xlabel:'n',pad:{l:40,r:16,t:12,b:26},xtarget:4,ytarget:2});
          a.stem(disc(n=>(n>=2&&n<=3)?1:0,-1,6),{color:C.mid,r:3}); a.note(5.6,2.0,'h[n−2]',{anchor:'end',color:C.mid,fs:12}); return a.svg(); }}],
        [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:410,h:150,xr:[-1,6],yr:[-0.3,2.4],xlabel:'n',pad:{l:40,r:16,t:12,b:26},xtarget:4,ytarget:2});
          a.stem(disc(n=>(n>=3&&n<=4)?2:0,-1,6),{color:C.mid,r:3}); a.note(5.6,2.0,'2h[n−3]',{anchor:'end',color:C.mid,fs:12}); return a.svg(); }}]
      ]}]},
    {t:'reveal', at:2, items:[
      {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:210,xr:[-1,6],yr:[-0.4,3.8],xlabel:'n',pad:{l:46,r:22,t:16,b:30},xtarget:7,ytarget:3});
        ...
        a.note(5.6,3.4,'y[n]',{anchor:'end',color:C.out,fs:15,italic:true}); return a.svg(); },
        caption:'Redrawn from the source figure (PDF p. 16). Numerically verified.'}]}
  ]}
]},
...
{ id:'m3-ex-dt2', module:'M3', nav:'Worked example · geometric', title:'Worked example — an infinite-length case', src:'pp. 16–17',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'reveal', at:1, items:[
      {t:'note', kind:'def', head:'Case I · $n<0$', html:'$x[k]$ lives on $k\\ge0$, $h[n-k]$ on $k\\le n<0$. The two supports do not overlap, so every product is zero:<br>$y[n]=0$ for $n<0$.'}]},
    {t:'reveal', at:2, items:[
      {t:'note', kind:'def', head:'Case II · $n\\ge0$', html:'The overlap is $0\\le k\\le n$, so $\\displaystyle y[n]=\\sum_{k=0}^{n}\\left(\\tfrac12\\right)^{k}$.'},
      {t:'eq', size:'sm', tex:'\\sum_{k=m}^{n}a\\,r^{k}=\\frac{a\\bigl(r^{m}-r^{n+1}\\bigr)}{1-r}',
        label:'Finite geometric sum',
        note:'The source states this with the condition $|r|<1$. In fact the <b>finite</b> sum requires only $r\\neq1$; $|r|<1$ is needed for the <em>infinite</em> sum. <span class="small">(Ambiguity A-07 in the source ledger — corrected here and recorded.)</span>'}]},
    {t:'reveal', at:3, items:[
      {t:'eq', size:'sm', tex:'y[n]=\\frac{\\left(\\tfrac12\\right)^{0}-\\left(\\tfrac12\\right)^{n+1}}{1-\\tfrac12}=2\\left(1-\\tfrac12\\left(\\tfrac12\\right)^{n}\\right)=2-\\left(\\tfrac12\\right)^{n}'},
    ...
    {t:'reveal', at:4, items:[
      {t:'wex', rows:[
        ...
        ['Sanity check 2','As $n\\to\\infty$, $y[n]\\to2=\\sum_{k\\ge0}(1/2)^{k}$ — the accumulator has summed the entire input. ✓'],
        ['Interpretation','$h[n]=u[n]$ is the accumulator of Module 2. Convolving with it produces the running sum, and because the input is absolutely summable the running sum converges — a bounded output from an unstable system, which is allowed: BIBO stability requires <em>every</em> bounded input to give a bounded output, not merely this one.']
      ]}]}
  ], right:[
    {t:'grid', cols:1, gap:'6px', items:[
      [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:180,xr:[-2,11],yr:[-0.2,1.3],xlabel:'n',pad:{l:46,r:22,t:14,b:28},xtarget:7,ytarget:2});
        a.stem(disc(n=>n>=0?Math.pow(0.5,n):0,-2,11),{color:C.in}); a.note(10.6,1.14,'x[n]',{anchor:'end',color:C.in,fs:14,italic:true}); return a.svg(); }}],
      [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:180,xr:[-2,11],yr:[-0.2,1.3],xlabel:'n',pad:{l:46,r:22,t:14,b:28},xtarget:7,ytarget:2});
        a.stem(disc(n=>n>=0?1:0,-2,11),{color:C.h}); a.note(10.6,1.14,'h[n] = u[n]',{anchor:'end',color:C.h,fs:14,italic:true}); return a.svg(); }}]
    ]},
    {t:'reveal', at:3, items:[
      {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:250,xr:[-2,11],yr:[-0.2,2.4],xlabel:'n',pad:{l:48,r:22,t:18,b:32},xtarget:7,ytarget:3});
        ...
        a.note(10.6,2.18,'y[n] → 2',{anchor:'end',color:C.coral,fs:15});
        return a.svg(); },
        caption:'Redrawn from the source figure (PDF p. 17). Verified numerically against the convolution sum.'}]}
  ]}
]},
...
{ id:'m3-convint', module:'M3', nav:'The convolution integral', title:'The convolution integral', src:'pp. 17–18',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'reveal', at:1, items:[
      {t:'body', html:'This is the continuous-time representation property — $x$ as a <em>continuum</em> of weighted, shifted impulses. Feeding it through an LTI system and repeating the three steps of the discrete case gives:'},
    ...
    {t:'reveal', at:2, items:[
      {t:'note', kind:'warn', head:'Building $h(t-\\tau)$ correctly', html:'The source works it as “shift, then scale” in the variable $\\tau$. Example: to plot $\\delta(-t+5)$, first $v(t)=\\delta(t+5)$, then $y(t)=v(-t)=\\delta(-t+5)$ — an impulse at $t=5$. The same routine builds $h(t-\\tau)$, and it is where sign errors are made if it is done by eye.'}]}
  ], right:[
    {t:'fig', frame:true, svg:()=>{
      ...
      a.note(-5,1.22,'v(t) = δ(t+5)',{anchor:'middle',color:C.mid,fs:14,italic:true});
      return a.svg(); }},
    {t:'fig', frame:true, svg:()=>{
      const a=P.Axes({w:820,h:230,xr:[-8,8],yr:[-0.25,1.35],xlabel:'t',pad:{l:48,r:24,t:20,b:34},xtarget:9,ytarget:2});
      a.impulse(5,1,{color:C.out,labelText:'1'});
      a.note(5,1.22,'y(t) = v(−t) = δ(−t+5)',{anchor:'middle',color:C.out,fs:14,italic:true});
      return a.svg(); },
      caption:'Redrawn from the source mini-example (PDF p. 17): shift first, then reverse. The impulse ends up at $t=+5$, not $t=-5$.'},
    {t:'reveal', at:1, items:[
      {t:'fig', frame:true, svg:()=>{
        const a=P.Axes({w:820,h:250,xr:[-1,7],yr:[-0.15,1.2],xlabel:'τ',pad:{l:48,r:24,t:20,b:34},xtarget:8,ytarget:2});
        ...
        a.note(6.6,1.05,'x(τ) as a continuum of impulses',{anchor:'end',color:C.muted,fs:14});
        return a.svg(); },
        caption:'The continuous-time representation property, illustrated. Each impulse carries weight $x(\\tau)\\d\\tau$; the system responds to each with $h(t-\\tau)$ and the integral superposes them. <b>Editorial illustration</b>.'}]}
  ]}
]},
...
{ id:'m3-ex-ct1', module:'M3', nav:'Worked example · CT case split', title:'Worked example — a two-case convolution', src:'pp. 18–19',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    {t:'wex', rows:[
      ...
      ['Method','$y(t)=\\int x(\\tau)h(t-\\tau)\\d\\tau$. Note $x(\\tau)$ is non-zero for $\\tau\\le0$ and $h(t-\\tau)=u(t-\\tau-3)$ is non-zero for $\\tau\\le t-3$. The overlap therefore ends at $\\min(0,\\,t-3)$ — which is what creates the two cases.']
    ]},
    {t:'reveal', at:1, items:[
      {t:'note', kind:'def', head:'Case I · $t-3<0$, i.e. $t<3$', html:'The binding limit is $t-3$:<br>$y(t)=\\displaystyle\\int_{-\\infty}^{t-3}e^{2\\tau}\\,\\d\\tau=\\left[\\tfrac12 e^{2\\tau}\\right]_{-\\infty}^{t-3}=\\tfrac12 e^{2(t-3)}$.'}]},
    {t:'reveal', at:2, items:[
      {t:'note', kind:'def', head:'Case II · $t-3>0$, i.e. $t>3$', html:'Now the binding limit is 0 — the whole of $x$ is covered:<br>$y(t)=\\displaystyle\\int_{-\\infty}^{0}e^{2\\tau}\\,\\d\\tau=\\tfrac12$.'},
    ...
    {t:'grid', cols:1, gap:'6px', items:[
      [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:180,xr:[-6,8],yr:[-0.1,1.2],xlabel:'t',pad:{l:48,r:22,t:14,b:28},xtarget:8,ytarget:2});
        a.curve(t=>t<=0?Math.exp(2*t):0,{color:C.in}); a.note(7.6,1.05,'x(t)',{anchor:'end',color:C.in,fs:14,italic:true}); return a.svg(); }}],
      [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:180,xr:[-6,8],yr:[-0.1,1.2],xlabel:'t',pad:{l:48,r:22,t:14,b:28},xtarget:8,ytarget:2});
        a.curve(t=>t>=3?1:0,{color:C.h}); a.note(7.6,1.05,'h(t) = u(t−3)',{anchor:'end',color:C.h,fs:14,italic:true}); return a.svg(); }}]
    ]},
    {t:'reveal', at:1, items:[
      {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:180,xr:[-6,8],yr:[-0.1,1.25],xlabel:'τ',pad:{l:46,r:20,t:14,b:28},xtarget:8,ytarget:2});
        const t=1.2;
        ...
        a.note(t-3,1.14,'τ = t − 3',{anchor:'middle',color:C.coral,fs:13});
    ...
    {t:'reveal', at:2, items:[
      ...
      {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:172,xr:[-1,7],yr:[-0.05,0.6],xlabel:'t',pad:{l:50,r:20,t:14,b:28},xtarget:8,ytarget:3});
        ...
        a.note(6.6,0.55,'y(t)',{anchor:'end',color:C.out,fs:15,italic:true}); return a.svg(); },
        caption:'Redrawn from the source figure (PDF p. 19). Verified symbolically and numerically.'}]}
  ]}
]},
...
{ id:'m3-ex-ct2', module:'M3', nav:'Worked example · five cases', title:'Worked example — a five-case convolution', src:'pp. 19–20',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    {t:'wex', rows:[
      ...
      ['Find','$y(t)=x(t)*h(t)$; determine and plot.'],
      ['Method','Use $y(t)=\\int h(\\tau)x(t-\\tau)\\d\\tau$ — flip the <em>rectangle</em>, since it is the simpler shape. Its support becomes $[t-1,\\,t]$, a window of width 1 sliding across the ramp on $[0,2]$.']
    ]},
    {t:'reveal', at:1, items:[
      {t:'note', kind:'def', head:'Boundaries come from the window edges meeting the ramp edges', html:'The window edges are $t-1$ and $t$; the ramp edges are $0$ and $2$. Equating them gives $t=0,\\,1,\\,2,\\,3$ — the four boundaries, hence five cases. No case can be found or lost by accident once this list is written down.'}]},
    {t:'reveal', at:2, items:[
      {t:'wex', rows:[
        ['I · $t<0$','No overlap ⇒ $y(t)=0$.'],
        ['II · $0<t<1$','$y(t)=\\displaystyle\\int_{0}^{t}\\tau\\,\\d\\tau=\\tfrac12 t^{2}$.'],
        ['III · $1<t<2$','$y(t)=\\displaystyle\\int_{t-1}^{t}\\tau\\,\\d\\tau=\\tfrac{t^{2}-(t-1)^{2}}{2}=t-\\tfrac12$.'],
        ['IV · $2<t<3$','$y(t)=\\displaystyle\\int_{t-1}^{2}\\tau\\,\\d\\tau=\\tfrac{4-(t-1)^{2}}{2}=-\\tfrac12 t^{2}+t+\\tfrac32$.'],
        ['V · $t>3$','No overlap ⇒ $y(t)=0$.']
      ]}]},
    {t:'reveal', at:4, items:[
      {t:'wex', rows:[
      ...
      {t:'instr', head:'Homework from the source', html:'Repeat with $x(t)=1$ on $0<t<3$ and the same $h$. The window is now <em>wider</em> than the ramp, which changes the middle case — a test of whether the boundary list was understood rather than memorised.'}]}
  ], right:[
    {t:'grid', cols:2, gap:'12px', items:[
      [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:410,h:150,xr:[-0.5,3.5],yr:[-0.15,2.3],xlabel:'t',pad:{l:40,r:16,t:12,b:26},xtarget:4,ytarget:2});
        a.curve(t=>(t>0&&t<1)?1:0,{color:C.in}); a.note(3.3,2.0,'x(t)',{anchor:'end',color:C.in,fs:13,italic:true}); return a.svg(); }}],
      [{t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:410,h:150,xr:[-0.5,3.5],yr:[-0.15,2.3],xlabel:'t',pad:{l:40,r:16,t:12,b:26},xtarget:4,ytarget:2});
        a.curve(t=>(t>0&&t<2)?t:0,{color:C.h}); a.note(3.3,2.0,'h(t)',{anchor:'end',color:C.h,fs:13,italic:true}); return a.svg(); }}]
    ]},
    {t:'reveal', at:2, items:[
      {t:'grid', cols:3, gap:'10px', items:[
        [0.6,'II'],[1.5,'III'],[2.5,'IV']
      ].map(([tv,lab])=>[{t:'fig', frame:true, svg:()=>{
        const a=P.Axes({w:400,h:158,xr:[-1.2,3.6],yr:[-0.2,2.3],xlabel:'τ',pad:{l:38,r:14,t:12,b:26},xtarget:4,ytarget:2});
    ...
    {t:'reveal', at:3, items:[
      ...
      {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:820,h:180,xr:[-0.5,3.5],yr:[-0.15,1.75],xlabel:'t',pad:{l:48,r:20,t:14,b:28},xtarget:5,ytarget:3});
        ...
        a.note(3.35,1.6,'y(t)',{anchor:'end',color:C.out,fs:15,italic:true});
        return a.svg(); },
        caption:'Peak $1.5$ at $t=2$, matching the source MATLAB figure. Verified symbolically with SymPy and numerically by quadrature.'}]}
  ]}
]},
...
{ id:'m3-lab-e', module:'M3', nav:'Laboratory E · Convolution', title:'Laboratory E — Graphical Convolution Explorer', src:'pp. 15–20',
  ...
  {t:'lede', text:'All four cases are the worked examples of this module. Drag the shift and watch the overlap create — and destroy — the case boundaries you derived by hand.'},
  {t:'lab', id:'E'}
]},
...
{ id:'m3-props', module:'M3', nav:'Convolution properties', title:'Properties of convolution', src:'p. 20',
  ...
  {t:'cols', ratio:'c-5-7', left:[
    {t:'eq', tex:'x[n]*h[n]=h[n]*x[n],\\qquad x(t)*h(t)=h(t)*x(t)', label:'(1) Commutative',
      note:'Flip whichever signal is easier. Physically: input and impulse response play symmetric roles in the algebra, even though they are entirely different objects physically.'},
    {t:'reveal', at:1, items:[
    ...
    {t:'reveal', at:2, items:[
      {t:'eq', tex:'x*(h_1*h_2)=(x*h_1)*h_2', label:'(3) Associative',
        note:'Two systems in <b>cascade</b> are equivalent to one system with impulse response $h_1*h_2$. Combined with commutativity, the cascade order is irrelevant — for LTI systems only.'}]},
    {t:'reveal', at:3, items:[
    ...
    {t:'fig', frame:true, svg:()=>P.blocks({w:860,h:210,items:[
      ...
      {t:'box',x:170,y:38,w:120,h:44,label:'h₁'},{t:'box',x:170,y:128,w:120,h:44,label:'h₂'},
      ...
      {t:'box',x:640,y:83,w:150,h:44,label:'h₁ + h₂'},
      {t:'arrow',x1:790,y1:105,x2:840,y2:105}
    ]}), caption:'Distributive property — parallel interconnection. Redrawn from the source diagram (PDF p. 20).'},
    {t:'reveal', at:2, items:[
      {t:'fig', frame:true, svg:()=>P.blocks({w:860,h:150,items:[
        {t:'arrow',x1:40,y1:75,x2:130,y2:75},{t:'box',x:130,y:53,w:110,h:44,label:'h₁'},
        {t:'arrow',x1:240,y1:75,x2:320,y2:75},{t:'box',x:320,y:53,w:110,h:44,label:'h₂'},
        {t:'arrow',x1:430,y1:75,x2:500,y2:75},
        {t:'text',x:545,y:82,label:'≡',fs:26},
        {t:'arrow',x1:585,y1:75,x2:655,y2:75},
        {t:'box',x:655,y:53,w:150,h:44,label:'h₁ ∗ h₂'},
...
{ id:'m3-lti-props', module:'M3', nav:'LTI property criteria', title:'The six properties, re-expressed in $h$', src:'p. 21',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    {t:'eq', tex:'h(t)=a\\,\\delta(t)\\quad\\text{or}\\quad h[n]=a\\,\\delta[n],\\qquad a\\in\\mathbb{C}',
      label:'(4) Memoryless ⟺',
      note:'Proof: $y[n]=\\sum_k x[k]\\,a\\,\\delta[n-k]=a\\,x[n]$ — a pure gain. Any $h$ with a non-zero sample away from the origin introduces memory.'},
    {t:'reveal', at:1, items:[
    ...
    {t:'reveal', at:2, items:[
      {t:'eq', tex:'h(t)=0\\ \\text{for}\\ t<0\\quad\\text{or}\\quad h[n]=0\\ \\text{for}\\ n<0', label:'(6) Causal ⟺',
        note:'Proof: the sum collapses to $y[n]=\\sum_{k\\ge0}h[k]x[n-k]=h[0]x[n]+h[1]x[n-1]+\\cdots$ — present and past only.'}]},
    {t:'reveal', at:3, items:[
    ...
    {t:'reveal', at:4, items:[
      {t:'note', kind:'warn', head:'What the source proves, and what it asserts', html:'The proof above establishes <b>sufficiency</b>: absolute summability ⇒ BIBO stability. The source states the criterion as an equivalence. Necessity is a standard result (it follows from the bounded input $x[n]=\\operatorname{sgn}h[-n]$, which forces $y[0]=\\sum_k|h[k]|$) but is <em>not</em> proved in the notes. <span class="small">(Ambiguity A-08 in the source ledger.)</span>'}]}
  ], right:[
    {t:'fig', frame:true, svg:()=>P.blocks({w:820,h:150,items:[
      {t:'arrow',x1:40,y1:75,x2:150,y2:75},{t:'box',x:150,y:53,w:120,h:44,label:'h[n]'},
      {t:'arrow',x1:270,y1:75,x2:380,y2:75},{t:'box',x:380,y:53,w:120,h:44,label:'g[n]'},
      {t:'arrow',x1:500,y1:75,x2:610,y2:75},
      {t:'text',x:95,y:62,label:'x[n]',italic:true,fs:15},{t:'text',x:325,y:62,label:'y[n]',italic:true,fs:15},
      {t:'text',x:660,y:80,label:'x[n]',italic:true,fs:15},
      {t:'text',x:440,y:130,label:'inverse system',fs:12,color:'#4A657F'}
    ]}), caption:'Invertibility as a cascade that reduces to the identity, $h*g=\\delta$. Redrawn from the source diagram (PDF p. 21).'},
    {t:'reveal', at:3, items:[
...
{ id:'m3-synth', module:'M3', nav:'Module 3 synthesis', title:'Module 3 — what to carry forward', src:'pp. 14–21',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'lede', text:'Convolution is complete, general, and painful. Five cases for two elementary shapes is not a method that scales. So: is there a class of input signals for which an LTI system does something simpler than convolve?'},
    {t:'reveal', at:2, items:[
      ...
      {t:'body', html:`<p style="color:#DED5C6">The signal comes out unchanged in shape, scaled by a constant. Complex exponentials are the <b>eigenfunctions</b> of LTI systems — and that single observation is the whole of Modules 4 to 7.</p>`}]}
  ]}
]},

{ id:'m3-qbank', module:'M3', nav:'Module 3 question bank', title:'Module 3 — question bank', src:'pp. 14–21',
  objective:'Twelve editorially developed questions covering Module 3 outcomes.',
  ...
  {t:'small', html:'All questions are <b>editorially developed</b> and solvable with Modules 1–3 material only. Several are built specifically around the missing-flip error and around confusing the LTI property criteria with the general definitions of Module 2.'},
  {t:'qbank', module:'M3'}
]}
