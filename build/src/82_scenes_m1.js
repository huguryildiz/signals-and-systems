/* ==========================================================================
   EE311 · Module 1 — Signal Foundations            [Source: 2–10]
   ========================================================================== */
(function(){
...
{ id:'m1-open', module:'M1', nav:'Module 1 opening', title:'Signal Foundations', src:'pp. 2–10',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    {t:'body', html:`<p style="color:#CFC5B4">The module runs from the opening definition to the discrete-time periodicity condition. Two results here are the ones students most often get wrong later:</p>`},
...
{ id:'m1-def', module:'M1', nav:'Definitions and notation', title:'Definitions and notation', src:'p. 2',
  ...
  {t:'cols', ratio:'c-5-7', left:[
    ...
    {t:'grid', cols:1, gap:'10px', items:[
      ...
      }, caption:'The discrete signal is <b>only</b> the dots; the dashed intuition of a curve through them is a separate object and must be justified (Module 7 does exactly that).'}]
    ]}
  ]}
]},
...
{ id:'m1-power', module:'M1', nav:'Instantaneous power', title:'From circuit power to signal power', src:'p. 2',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'reveal', at:2, items:[
      {t:'note', kind:'warn', head:'The convention the definition adopts silently', html:'From here on the notes drop the $1/R$ and write $|x(t)|^{2}$. This is the <b>normalised</b> convention, $R=1\\ \\Omega$. It is standard, and it is why energies are quoted in joules without a resistance ever being named. Restore the $1/R$ whenever a physical number is required.'}]},
    {t:'reveal', at:3, items:[
    ...
    {t:'fig', frame:true, svg:()=>{
      ...
      a.span(1.2,4.4,1.30,'energy = shaded area',{color:C.coral});
      return a.svg(); },
...
{ id:'m1-avgpower', module:'M1', nav:'Average power', title:'Average power', src:'p. 2',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'reveal', at:1, items:[
      {t:'note', kind:'def', head:'Why $2N+1$ and not $2N$', html:'$2N+1$ is the <b>number of elements</b> between $-N$ and $+N$ inclusive. Using $2N$ gives the same limit but the wrong finite-$N$ value, and the difference shows up immediately in short-record estimates.'}]},
    {t:'reveal', at:2, items:[
...
{ id:'m1-classify', module:'M1', nav:'Energy, power, or neither', title:'Energy, power, or neither', src:'p. 3',
  ...
  {t:'cols', ratio:'c-5-7', left:[
    ...
    {t:'reveal', at:1, items:[
      {t:'note', kind:'err', head:'Neither — the case the two lines above do not cover', html:'If a signal grows without bound, <em>both</em> quantities diverge. $x(t)=t\\,u(t)$ has $E_\\infty\\to\\infty$ and $P_\\infty\\to\\infty$. It is neither an energy nor a power signal.'}]},
    {t:'reveal', at:2, items:[
...
{ id:'m1-shift', module:'M1', nav:'Time shifting', title:'Time shifting', src:'p. 3',
  ...
  {t:'cols', ratio:'c-4-8', vcenter:true, left:[
    ...
    {t:'fig', frame:true, svg:()=>{
      ...
      return a.svg(); },
      caption:'The pulse shape is untouched; only its position on the time axis changes.'}
  ]}
]},
...
{ id:'m1-reverse-scale', module:'M1', nav:'Reversal and scaling', title:'Reversal and scaling', src:'pp. 3–4',
  ...
  {t:'cols', ratio:'c-5-7', left:[
    {t:'eq', tex:'x(t)\\;\\longrightarrow\\;x(-t)\\qquad\\bigl(x[n]\\to x[-n]\\bigr)', label:'Time reversal',
      note:'A reflection about the vertical axis. It is a flip about the vertical axis.'},
    {t:'reveal', at:1, items:[
      ...
      {t:'note', kind:'warn', head:'A notation caution', html:'The source heading writes “$x(t)=x(at)$”. Read this as the definition of a <em>new</em> signal, $y(t)=x(at)$: the equation as written would force $a=1$.'}]},
    {t:'reveal', at:2, items:[
...
{ id:'m1-periodic', module:'M1', nav:'Periodicity', title:'Periodicity', src:'p. 5',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'reveal', at:3, items:[
      {t:'note', kind:'def', head:'Why "smallest" matters', html:'If $T$ is a period then so is every $kT$. For example, take $T=4,8,12,\\dots$ and fixes $T_0=4$. Without the minimality clause $\\omega_0$ would not be well defined, and the harmonic indexing of Module 4 would collapse.'}]}
  ], right:[
    {t:'fig', frame:true, svg:()=>{
      ...
      a.span(8,12,1.12,'T₀ = 4',{color:C.coral}); a.span(8,16,1.60,'T = 8',{color:C.muted});
      return a.svg(); },
      caption:''},
    {t:'fig', frame:true, svg:()=>{
  ...
  {t:'cols', ratio:'c-5-7', left:[
    ...
    {t:'reveal', at:1, items:[
      {t:'fig', frame:true, svg:()=>{
        ...
        a.note(-0.25,-1.5,'Od{x} = −sinh t',{anchor:'end',color:C.mid,fs:14});
        return a.svg(); },
...
{ id:'m1-dt-sift', module:'M1', nav:'Sampling and sifting (DT)', title:'Sampling and sifting properties', src:'pp. 6–7',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:840,h:230,xr:[-1,5],yr:[-0.3,3.6],xlabel:'n',pad:{l:48,r:24,t:20,b:34},xtarget:7,ytarget:3});
      ...
      caption:'Note that the product is a <em>sequence</em>; the sifting property is the number obtained by summing it.'}
  ]}
]},
...
{ id:'m1-ct-impulse', module:'M1', nav:'CT impulse and step', title:'The continuous-time impulse', src:'p. 7',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'reveal', at:1, items:[
      {t:'note', kind:'err', head:'Why this definition cannot be taken literally', html:'No ordinary function is zero almost everywhere and still integrates to 1 — the Lebesgue integral of such a function is 0. $\\delta$ is a <b>distribution</b> (a generalized function): an object defined by what it <em>does</em> to well-behaved test functions, namely $\\int x(t)\\delta(t-t_0)\\d t=x(t_0)$.'}]},
    {t:'reveal', at:2, items:[
      {t:'body', html:'A cleaner construction: a rectangle of width $\\varepsilon$ and height $1/\\varepsilon$ — area 1 for every $\\varepsilon$ — and lets $\\varepsilon\\to0$. The limit does not exist pointwise; it exists only under an integral sign, which is precisely the distributional statement.'}]},
    {t:'reveal', at:3, items:[
    ...
    {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:840,h:240,xr:[-1.2,1.2],yr:[-0.4,4.6],xlabel:'t',pad:{l:48,r:24,t:22,b:34},xtarget:5,ytarget:3});
      ...
      return a.svg(); },
      caption:'The limiting construction : each rectangle has unit area. Nothing converges pointwise; the family converges only when integrated against a continuous test function.'},
    {t:'reveal', at:4, items:[
      {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:840,h:230,xr:[-1,6],yr:[-1.1,1.6],xlabel:'t',pad:{l:48,r:24,t:22,b:34},xtarget:7,ytarget:3});
        ...
        a.note(2.86,-0.28,'t₀',{anchor:'end',color:C.coral,fs:14});
        return a.svg(); },
  ...
  {t:'cols', ratio:'c-5-7', left:[
    ...
    {t:'reveal', at:4, items:[
      {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:900,h:260,xr:[0,5],yr:[-2.3,2.3],xlabel:'t',pad:{l:50,r:26,t:20,b:36},xtarget:6,ytarget:3});
        ...
        return a.svg(); },
        caption:'5$): a sinusoid confined by the envelope $\\pm Ae^{rt}$.'}]}
  ]}
]},
  ...
  {t:'cols', ratio:'c-5-7', left:[
    ...
    {t:'reveal', at:2, items:[
      {t:'eq', tex:'C=|C|e^{j\\theta},\\;\\alpha=|\\alpha|e^{j\\omega_0}\\;\\Rightarrow\\; x[n]=|C||\\alpha|^{n}\\cos(\\omega_0 n+\\theta)+j\\,|C||\\alpha|^{n}\\sin(\\omega_0 n+\\theta)',
        ...
        note:'$|\\alpha|=1$ sustained, $|\\alpha|>1$ growing, $|\\alpha|<1$ decaying — the definition shows $\\alpha=1,\\,1.05,\\,0.95$ at $\\omega_0=0.14\\pi$.'}]}
  ], right:[
...
{ id:'m1-dt-period', module:'M1', nav:'DT periodicity condition', title:'When is a discrete-time exponential periodic?', src:'p. 10',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:900,h:270,xr:[-20,20],yr:[-1.35,1.5],xlabel:'n',ylabel:'\\operatorname{Re}\\{x[n]\\}',pad:{l:60,r:26,t:22,b:36},xtarget:9,ytarget:3});
      ...
      return a.svg(); },
      caption:'$x[n]=e^{j3\\pi n/5}$, real part. '},
    {t:'reveal', at:2, items:[
      {t:'fig', frame:true, svg:()=>{ const a=P.Axes({w:900,h:250,xr:[-20,20],yr:[-1.35,1.35],xlabel:'n',pad:{l:52,r:26,t:20,b:36},xtarget:9,ytarget:3});
        ...
        return a.svg(); },
        caption:'<b>Aperiodic.</b> $x[n]=\\cos(n)$ has $\\omega_0=1$, so $\\omega_0/2\\pi=1/(2\\pi)$ is irrational — the pattern never exactly repeats. It looks periodic; it is not.'}]}
  ]}
]},
