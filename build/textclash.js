/* Text/geometry collision sweep for every figure in the artifact.
   ...
   A label is either an svg <text> or an axis name, which is typeset mathematics
   in a foreignObject; the latter is measured through its laid-out box and mapped
...
const GRID = ['#E2DACA', '#2C2820', '#D9D0BE', '#BFB39B', '#E4DCCA'];  /* grid + rule tokens: guides, not content */
...
(async () => {
    ...
    for (let st = 0; st <= (s.steps || 0); st++) {
      ...
      const found = await page.evaluate(({ GRID, AXIS }) => {
        ...
        svgs.forEach((svg, si) => {
          ...
          /* labels: plain svg text, plus the typeset axis names */
          ...
          svg.querySelectorAll('foreignObject[data-axisname] .katex').forEach(k => {
            if (!inv) return;
          ...
          const geos = Array.from(svg.querySelectorAll('path,line,circle,rect,polyline,polygon'))
            .filter(el => !el.closest('foreignObject'));
          const cls = el => {
          ...
          const LIGHTFILL = ['#FCF9F3', '#F7F2E8', '#FFFFFF', '#EFE8DA', '#F2EDE1', '#BFB39B',
                             '#D9D0BE', '#E2DACA', '#1F2A33', '#16232F', 'NONE', ''];
          const geo = geos.map(el => ({ el, k: cls(el) })).filter(g => g.k !== 'grid')
            .map(g => {
              ...
              return Object.assign(g, { solid: !LIGHTFILL.includes(f) && !faint });
            });
          ...
          labels.forEach((t, i) => {
            ...
            if (content || tt || cut || axis > total * 0.10) {
  ...
  for (const r of all) {
    ...
    const p = seen.get(k);
    const worse = !p || (!!r.cut && !p.cut) || (!!r.cut === !!p.cut && r.cf > p.cf);
    if (worse) seen.set(k, r);
  }
  const rows = Array.from(seen.values()).sort((a, b) => b.cf - a.cf);
  /* A tick number or a one-symbol axis name crossed by a curve is acceptable:
     the halo cuts the curve around the glyphs and both stay readable. A word,
     an equation or a caption running under a signal is not, and neither is one
     label sitting on another, nor any label drawn without a halo, nor any label
     reaching past the edge of the figure, which cuts it off. */
  const numeric = t => /^[-−]?[\d.,]+$/.test(t.trim());
  const isSoft = r => r.halo && !r.tt && !r.cut && (numeric(r.text) || r.text.trim().length <= 2);
  const hard = rows.filter(r => (r.cf > 0 || r.tt || r.cut) && !isSoft(r));
  const soft = rows.filter(r => (r.cf > 0 || r.tt || r.cut) && isSoft(r));
  for (const r of hard) {
