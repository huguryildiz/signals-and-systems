...
/* --- original focal motif: a signal entering a system and leaving as a
       spectrum. Mathematically meaningful, not decorative. --- */
function motifSignalSystem(){
  ...
  a.curve(t=> t<9 ? Math.cos(1.6*t)+0.45*Math.cos(4.1*t+1) : null, {color:C.in,width:2.4});
  a.raw(`<rect x="${a.sx(10.2).toFixed(1)}" y="${a.sy(0.95).toFixed(1)}"
  ...
  a.note(13,-0.42,'h(t)',{anchor:'middle',fs:17,color:C.h,italic:true});
  a.raw(`<line x1="${a.sx(9.1)}" y1="${a.sy(0)}" x2="${a.sx(10.0)}" y2="${a.sy(0)}" stroke="${C.ink}" stroke-width="1.4"/>
         <path d="M${a.sx(10.2)},${a.sy(0)} l-9,-4.5 v9 Z" fill="${C.ink}"/>
         <line x1="${a.sx(15.8)}" y1="${a.sy(0)}" x2="${a.sx(16.8)}" y2="${a.sy(0)}" stroke="${C.ink}" stroke-width="1.4"/>
         <path d="M${a.sx(17.0)},${a.sy(0)} l-9,-4.5 v9 Z" fill="${C.ink}"/>`);
  [[18.6,1.05],[20.2,0.62],[21.8,0.30],[23.4,0.14],[25.0,0.06]].forEach(([x,h])=>{
    a.raw(`<line x1="${a.sx(x)}" y1="${a.sy(0)}" x2="${a.sx(x)}" y2="${a.sy(h)}" stroke="${C.out}" stroke-width="2.4"/>
           <circle cx="${a.sx(x)}" cy="${a.sy(h)}" r="4" fill="${C.out}"/>`);
  });
...
{ id:'title', module:'M0', nav:'Title', title:'EE 311 — Signals and Systems', src:'p. 1',
  ...
  {t:'stack', style:'justify-content:center;flex:1;align-items:flex-start', items:[
    ...
    {t:'raw', html:`<div style="margin:26px 0 30px">${motifSignalSystem()}</div>`},
    {t:'raw', html:`<div style="display:flex;gap:56px;align-items:flex-start;font-size:16px;color:var(--muted);
      ...
      <div><b style="color:var(--graphite)">Source</b><br>EE311 — Lecture Notes.pdf (88 pp.)</div>
      <div><b style="color:var(--graphite)">Version</b><br>v0.9 · Phase 1 (Modules 0–3)</div>
...
{ id:'m0-signal', module:'M0', nav:'What a signal represents', title:'What a signal represents', src:'p. 2',
  ...
  {t:'cols', ratio:'c-5-7', vcenter:true, left:[
    {t:'body', html:'The lecture notes open with two sentences that do more work than they appear to:'},
    {t:'note', kind:'def', head:'Definition (source)', html:'A signal is a <b>physical variation that carries information</b>. A signal is a <b>function of one or more independent variables</b>.'},
    {t:'reveal', at:1, items:[
    ...
    {t:'fig', frame:true, svg:()=>{
      const a=P.Axes({w:660,h:300,xr:[0,10],yr:[-1.6,1.6],xlabel:'t  (independent variable)',ylabel:'x(t)',
...
{ id:'m0-system', module:'M0', nav:'What a system does', title:'What a system does', src:'p. 11',
  ...
  {t:'cols', ratio:'c-6-6', vcenter:true, left:[
    {t:'note', kind:'def', head:'Definition (source)', html:'A system is a <b>quantitative description of a physical process</b> which transforms input signals into output signals — a “black box” that <b>deterministically</b> maps one signal to another.'},
...
{ id:'m0-howto', module:'M0', nav:'Using this artifact', title:'How to use this artifact', src:'—',
  objective:'Explain modes, controls and the source-citation policy.',
  ...
  {t:'cols', ratio:'c-6-6', left:[
    ...
    {t:'sub', text:'Source citation policy'},
    {t:'body', html:`<p>Every scene carries a reference of the form <code>[Source: EE311 Lecture Notes, PDF p. XX]</code>. Material that is <em>not</em> in the lecture notes is labelled an <b>editorial enhancement</b> and never presented as source content. Question banks are labelled as editorially developed.</p>`},
    {t:'sub', text:'Conventions fixed for the whole artifact'},
    {t:'body', html:`<p>Energy and power are <b>normalised</b> ($R=1\\ \\Omega$). The imaginary unit is written $j$ (the source handwriting uses a capital J). Angular frequency $\\omega$ is in rad/s (continuous time) or rad/sample (discrete time); frequency in hertz is written explicitly where used.</p>`},
