...
- `.claude/hooks/agent_guard.py` blocks common `npm install` calls and direct agent edits to `dist/` through the configured `PreToolUse` hook. It is a narrow guard, not a substitute for the gates.
