...
function conceptMap(){
  ...
  COURSE_PATH.forEach((n,i)=>{
    ...
    const title=lines.map((line,j)=>`<text class="course-node-title" ${MAPHALO} x="${x.toFixed(1)}" y="${(y-(lines.length===1?3:14)+j*22).toFixed(1)}" text-anchor="middle">${line}</text>`).join('');
    const idY=y+(lines.length===1?21:35);
