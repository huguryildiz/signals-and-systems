...
const PLOT = (() => {
  ...
  let FEATHER = false; /* the soft outer edge of a TeX label's halo: the app turns it
                          on through setTheme; the printed notes never call setTheme
                          and leave it off, because a PDF stores every blurred edge
                          as its own raster mask (about a quarter of the notes' size) */
  ...
  /* Safari and every browser on an iPad paint a positioned element inside a
     foreignObject as if the svg were drawn at its own size: the label leaves
     its place whenever the figure is scaled, which it always is. KaTeX places
     the rows of a sub- or superscript with position:relative and a top offset.
     The same rows are laid out here in normal flow, each one moved by a top
     margin equal to the step from the row above (the rows have no height), so
     the label sets exactly as before and holds no positioned element. */
  const FLOW = ['base','vlist','delimcenter','op-symbol','rule','accent-body'];
  function flatTeX(html){
    if(typeof document === 'undefined') return html;
    const t = document.createElement('template'); t.innerHTML = html;
    t.content.querySelectorAll('.vlist').forEach(v=>{
      let prev = 0;
      for(const row of v.children){
        const m = /top:\s*(-?[\d.]+)em/.exec(row.getAttribute('style')||'');
        const top = m ? +m[1] : 0;
        row.style.removeProperty('top');
        row.style.marginTop = +(top-prev).toFixed(5)+'em';
        row.style.position = 'static';
        prev = top;
      }
    });
    /* an accent is set sideways by a left offset: the same shift as margins
       that give back to the right what they take on the left */
    t.content.querySelectorAll(FLOW.map(c=>'.'+c).join(',')).forEach(e=>{
      /* a top below a hundredth of an em (the lift of a small integral) is not seen */
      if(e.style.top && Math.abs(parseFloat(e.style.top))<0.01) e.style.removeProperty('top');
      if(e.style.top) return;
      const m = /^(-?[\d.]+)em$/.exec(e.style.left);
      if(m){
        const em = v => parseFloat(v)||0;
        e.style.marginLeft = +(em(e.style.marginLeft)+(+m[1])).toFixed(5)+'em';
        e.style.marginRight = +(em(e.style.marginRight)-(+m[1])).toFixed(5)+'em';
        e.style.removeProperty('left');
      } else if(e.style.left) return;
      e.style.position = 'static'; });
    /* a root sign is an svg cut to its box; the svg clips itself, so the box
       needs neither a position nor a clip of its own. Its width is taken from
       the box and never given to it, as when it stood outside the flow. */
    t.content.querySelectorAll('.hide-tail').forEach(e=>{
      e.style.position = 'static'; e.style.overflow = 'visible';
      for(const g of e.children) if(g.tagName.toLowerCase()==='svg'){
        g.style.position = 'static'; g.style.width = '0'; g.style.minWidth = '100%'; }
    });
    return t.innerHTML;
  }
  function texName(src, opts){
    ...
    let html;
    try { html = flatTeX(katex.renderToString(String(src), TEXOPT)); }
    catch(e){
    ...
    const ring = [[r,0],[-r,0],[0,r],[0,-r],[d,d],[d,-d],[-d,d],[-d,-d]]
      .map(([a,b]) => `${a}px ${b}px 0 ${c}`).join(',') + (FEATHER ? `,0 0 ${r}px ${c}` : '');
