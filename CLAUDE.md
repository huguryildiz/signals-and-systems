...
- `.claude/settings.json` and `.codex/hooks.json` call the same repository guards: `agent_guard.py` blocks common `npm install` calls and direct agent edits to `dist/` before a tool runs; `stop_guard.py` checks student-facing prose when a turn ends. Codex requires review and trust of new hook definitions before it runs them. These agent hooks are not CI gates and do not assess derivation completeness.
