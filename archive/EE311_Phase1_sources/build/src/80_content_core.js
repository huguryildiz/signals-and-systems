/* ==========================================================================
   EE311 · COURSE CONTENT — verified against `EE311 - Lecture Notes.pdf`.
...
const CONTENT = {
  ...
  META: {
    ...
    source:'EE311 - Lecture Notes.pdf (88 pp.)',
    version:'v0.9 · Phase 1 (Modules 0–3)',
    date:'2026-07-25',
  ...
  GLOSS: {
    ...
    ht:{ s:'h(t), h[n]', d:'Impulse response — the response of an LTI system to a unit impulse.', go:'m3-impulse' },
    dt:{ s:'δ(t)', d:'Continuous-time unit impulse (Dirac delta): a generalized function defined by its sifting action.', go:'m1-ct-impulse' },
  ...
  PROPS: [
    { k:'mem',  name:'Memoryless',
      crit:'The output at time $t$ (or $n$) depends only on the input at that same time. Any dependence on $x(t\\pm\\tau)$, $\\tau\\neq0$, or on past outputs, destroys it.' },
    { k:'inv',  name:'Invertible',
      crit:'Distinct inputs must produce distinct outputs. Prove it by exhibiting an inversion formula; disprove it with two inputs mapping to one output.' },
    { k:'caus', name:'Causal',
      crit:'The output at time $t$ (or $n$) depends only on inputs at times $\\le t$ (present and past).' },
    { k:'stab', name:'BIBO stable',
      crit:'Every bounded input, $|x|\\le B<\\infty$, must produce a bounded output. A single bounded input with unbounded output disproves it.' },
    { k:'ti',   name:'Time invariant',
      crit:'If $x(t)\\to y(t)$, then $x(t-t_0)\\to y(t-t_0)$ for every $t_0$. Compare the shifted-input response with the shifted output.' },
    { k:'lin',  name:'Linear',
      crit:'$a x_1 + b x_2 \\to a y_1 + b y_2$ for all $a,b\\in\\mathbb{C}$ — additivity and homogeneity together.' }
  ],

  /* ---- system catalogue for Laboratory D; every entry from the source ---- */
  SYSTEMS: [
    { tex:'y(t)=\\bigl[\\,2x(t)-x^{2}(t)\\,\\bigr]^{2}', src:'p. 11', p:{
      mem:{v:true, arg:'Only $x(t)$ appears — no $x(t+1)$, no $x(t-2)$, no integral. The output at $t$ is a fixed function of the input at $t$.'},
      ...
      stab:{v:true, arg:'If $|x(t)|\\le B$ then $|2x-x^{2}|\\le 2B+B^{2}$, so $|y|\\le(2B+B^{2})^{2}<\\infty$.'},
    ...
    { tex:'y[n]=x[n]+y[n-1]', src:'p. 11', p:{
      ...
      ti:{v:true,  arg:'The recursion has constant coefficients and no explicit $n$; delaying the input delays the whole solution (at rest).'},
    ...
    { tex:'y(t)=\\bigl[\\cos(t)+2\\bigr]\\,x(t)', src:'p. 12', p:{
      ...
      inv:{v:true, arg:'Since $\\cos(t)+2\\ge1>0$ for every $t$, the gain never vanishes and $x(t)=\\dfrac{y(t)}{\\cos(t)+2}$.'},
      caus:{v:true, arg:'Memoryless ⇒ causal.'},
      stab:{v:true, arg:'$|y(t)|\\le 3|x(t)|\\le 3B$.'},
      ti:{v:false, arg:'The gain depends explicitly on $t$. With $x(t)=1$: $y_1(t)=\\cos t+2$, but the response to $x(t-t_0)=1$ is again $\\cos t+2$, whereas $y_1(t-t_0)=\\cos(t-t_0)+2$. These differ for $t_0\\neq2\\pi k$.'},
    ...
    { tex:'y(t)=x^{2}(t)', src:'p. 12', p:{
      ...
      inv:{v:false,arg:'Counterexample from the source: $x_1(t)=1$ and $x_2(t)=-1$ are distinct inputs, yet $y_1(t)=y_2(t)=1$.'},
      caus:{v:true, arg:'Memoryless ⇒ causal.'},
      stab:{v:true, arg:'$|y|\\le B^{2}<\\infty$.'},
    ...
    { tex:'y[n]=x[-n]', src:'p. 12', p:{
      ...
      caus:{v:false,arg:'Source counterexample: $y[-1]=x[1]$ — the output at $n=-1$ requires a future input.'},
    ...
    { tex:'y(t)=\\int_{-\\infty}^{t}x(\\tau)\\,d\\tau', src:'p. 12', p:{
      ...
      ti:{v:true,  arg:'A change of variable shows the integrator commutes with delay.'},
    ...
    { tex:'y(t)=2x^{2}(t-1)+x(3t)', src:'p. 12', p:{
      ...
      inv:{v:false,arg:'The squared term destroys sign information; e.g. inputs that differ only in the sign of their $t-1$ contribution can collide.'},
      caus:{v:false,arg:'For $t<0$, $3t<t$ is fine, but for $t>0$, $x(3t)$ needs a future value: at $t=1$ the output uses $x(3)$.'},
      stab:{v:true, arg:'Source proof: $|y|\\le 2|x^{2}(t-1)|+|x(3t)|\\le 2B^{2}+B<\\infty$ by the triangle inequality.'},
    ...
    { tex:'y[n]=\\sum_{k=-\\infty}^{n}x[k]', src:'p. 13', p:{
      ...
      stab:{v:false,arg:'Source proof: $x[n]=u[n]$ gives $|x[n]|\\le1$ but $y[n]=\\sum_{k=0}^{n}1=n+1\\to\\infty$.'},
    ...
    { tex:'y(t)=\\sin\\bigl(x(t)\\bigr)', src:'p. 13', p:{
      ...
      ti:{v:true,  arg:'Source proof: $y_2(t)=\\sin(x_1(t-t_0))=y_1(t-t_0)$.'},
    ...
    { tex:'y[n]=n\\,x[n]', src:'p. 13', p:{
      ...
      ti:{v:false, arg:'Source counterexample: $x_1[n]=\\delta[n]\\Rightarrow y_1[n]=n\\delta[n]=0$; $x_2[n]=\\delta[n-1]\\Rightarrow y_2[n]=n\\delta[n-1]=\\delta[n-1]$. But $y_1[n-1]=0\\neq y_2[n]$.'},
    ...
    { tex:'y[n]=\\bigl(x[2n]\\bigr)^{2}', src:'p. 14', p:{
      ...
      inv:{v:false,arg:'Squaring loses sign, and decimation discards the odd-indexed samples entirely.'},
      caus:{v:false,arg:'At $n=1$ the output needs $x[2]$, a future sample.'},
      stab:{v:true, arg:'$|y[n]|\\le B^{2}$.'},
      ti:{v:false, arg:'Decimation is not time invariant: delaying the input by one sample does not delay $x[2n]$ by one sample.'},
      lin:{v:false,arg:'Source proof: $(a x_1[2n]+b x_2[2n])^{2}$ contains the cross term $2ab\\,x_1[2n]x_2[2n]$, which is absent from $a y_1[n]+b y_2[n]$.'} } },
    ...
    { tex:'y(t)=2\\pi\\,x(t)', src:'p. 14', p:{
      ...
      lin:{v:true, arg:'Source proof: $y_3=2\\pi(ax_1+bx_2)=a\\,2\\pi x_1+b\\,2\\pi x_2=ay_1+by_2$. This system is LTI.'} } }
  ],
