...
{t:'p', text:'Next, replace the summation index by $m=n-k$, so $k=n-m$. As $k$ runs over all integers, $m$ also runs over all integers, only in the opposite direction. The sum has the same terms, so its value is unchanged. This gives an equivalent form and shows that convolution is commutative:'},
{t:'eq', tex:'\\begin{aligned}\\sum_{k=-\\infty}^{\\infty}x[k]\\,h[n-k]&=\\sum_{m=-\\infty}^{\\infty}x[n-m]\\,h[n-(n-m)]\\\\&=\\sum_{m=-\\infty}^{\\infty}x[n-m]\\,h[m]\\;=\\;h[n]*x[n].\\end{aligned}'},
...
{t:'ex', hd:'Example 3.1', rows:[
 ...
 ['Solution','Start from the convolution sum. Only $k=0,1,2,3$ give non-zero $x[k]$, so the sum has four terms: $$\\begin{aligned}y[n]&=\\sum_{k=-\\infty}^{\\infty}x[k]\\,h[n-k]\\\\&=x[0]h[n]+x[1]h[n-1]+x[2]h[n-2]+x[3]h[n-3]\\\\&=h[n]+2h[n-1]+h[n-2]+2h[n-3].\\end{aligned}$$ Now evaluate at each $n$. Since $h[m]=1$ only for $m=0$ and $m=1$, the term $h[n-k]$ is non-zero only when $n-k=0$ or $n-k=1$. Keep only those terms: $$\\begin{aligned}y[0]&=x[0]h[0]=1\\cdot1=1,\\\\y[1]&=x[0]h[1]+x[1]h[0]=1\\cdot1+2\\cdot1=3,\\\\y[2]&=x[1]h[1]+x[2]h[0]=2\\cdot1+1\\cdot1=3,\\\\y[3]&=x[2]h[1]+x[3]h[0]=1\\cdot1+2\\cdot1=3,\\\\y[4]&=x[3]h[1]=2\\cdot1=2.\\end{aligned}$$ For $n<0$ and $n>4$ no term survives, so $y[n]=0$. This gives $y=\\{1,3,3,3,2\\}$ on $n=0,\\dots,4$.'],
...
{t:'ex', hd:'Example 3.2', rows:[
 ...
 ['Solution','Substitute the two signals into the convolution sum: $$y[n]=\\sum_{k=-\\infty}^{\\infty}x[k]\\,h[n-k]=\\sum_{k=-\\infty}^{\\infty}\\left(\\tfrac12\\right)^{k}u[k]\\,u[n-k].$$ The factor $u[k]$ is 1 only for $k\\ge0$. The factor $u[n-k]$ is 1 only for $n-k\\ge0$, that is, for $k\\le n$. Both conditions must hold for a term to survive.<br><b>Case 1, $n<0$.</b> The conditions $k\\ge0$ and $k\\le n<0$ cannot both hold. No term survives, so $y[n]=0$.<br><b>Case 2, $n\\ge0$.</b> The surviving terms are $0\\le k\\le n$, and on this range both step factors equal 1: $$y[n]=\\sum_{k=0}^{n}\\left(\\tfrac12\\right)^{k}.$$ This is a finite geometric sum with first term $1$, ratio $r=\\tfrac12$, and $n+1$ terms. Apply the formula in the box below with $a=1$, $m=0$: $$\\begin{aligned}y[n]&=\\frac{1-\\left(\\tfrac12\\right)^{n+1}}{1-\\tfrac12}\\\\&=\\frac{1-\\left(\\tfrac12\\right)^{n+1}}{\\tfrac12}=2\\Bigl(1-\\left(\\tfrac12\\right)^{n+1}\\Bigr)\\\\&=2-2\\left(\\tfrac12\\right)^{n+1}\\\\&=2-\\left(\\tfrac12\\right)^{n}.\\end{aligned}$$ The last line uses $2\\left(\\tfrac12\\right)^{n+1}=\\left(\\tfrac12\\right)^{n}$, because multiplying by 2 cancels one factor of $\\tfrac12$.<br>Together, $y[n]=\\left(2-\\left(\\tfrac12\\right)^{n}\\right)u[n]$.'],
...
{t:'p', text:'This form comes from the sifting definition in Chapter 1, $x(t_0)=\\int x(t)\\,\\delta(t-t_0)\\,\\d t$. Rename the integration variable $t$ as $\\tau$ and the sampling instant $t_0$ as $t$. This gives $x(t)=\\int x(\\tau)\\,\\delta(\\tau-t)\\,\\d\\tau$. The impulse is even, $\\delta(\\tau-t)=\\delta(t-\\tau)$, which gives the form above. Now apply the system to both sides. Additivity takes the integral outside the system, because an integral is a limit of sums. Homogeneity takes the weight $x(\\tau)\\,\\d\\tau$ outside, because it does not depend on $t$. Time invariance replaces $S\\{\\delta(t-\\tau)\\}$ by $h(t-\\tau)$:'},
...
{t:'ex', hd:'Example 3.3', rows:[
 ...
 ['Solution','Substitute the two signals into the convolution integral: $$y(t)=\\int_{-\\infty}^{\\infty}x(\\tau)\\,h(t-\\tau)\\,\\d\\tau=\\int_{-\\infty}^{\\infty}e^{2\\tau}u(-\\tau)\\,u(t-\\tau-3)\\,\\d\\tau.$$ The factor $u(-\\tau)$ is 1 only for $\\tau\\le0$. The factor $u(t-\\tau-3)$ is 1 only for $t-\\tau-3\\ge0$, that is, for $\\tau\\le t-3$. Both must hold, so the integrand is $e^{2\\tau}$ for $\\tau\\le\\min(0,\\,t-3)$ and zero elsewhere. The antiderivative of $e^{2\\tau}$ is $\\tfrac12e^{2\\tau}$, and $e^{2\\tau}\\to0$ as $\\tau\\to-\\infty$, so the lower limit contributes zero.<br><b>Case 1, $t<3$.</b> Here $t-3<0$, so the binding limit is $t-3$: $$\\begin{aligned}y(t)&=\\int_{-\\infty}^{t-3}e^{2\\tau}\\,\\d\\tau\\\\&=\\Bigl[\\tfrac12e^{2\\tau}\\Bigr]_{-\\infty}^{t-3}\\\\&=\\tfrac12e^{2(t-3)}-0=\\tfrac12 e^{2(t-3)}.\\end{aligned}$$ <b>Case 2, $t>3$.</b> Here $t-3>0$, so the binding limit is 0 and the whole of $x$ is covered: $$\\begin{aligned}y(t)&=\\int_{-\\infty}^{0}e^{2\\tau}\\,\\d\\tau\\\\&=\\Bigl[\\tfrac12e^{2\\tau}\\Bigr]_{-\\infty}^{0}\\\\&=\\tfrac12e^{0}-0=\\tfrac12.\\end{aligned}$$'],
...
{t:'ex', hd:'Example 3.4', rows:[
 ...
 ['Solution','Substitute into the chosen form: $$y(t)=\\int_{-\\infty}^{\\infty}h(\\tau)\\,x(t-\\tau)\\,\\d\\tau.$$ Here $h(\\tau)=\\tau$ for $0<\\tau<2$ and zero elsewhere. The factor $x(t-\\tau)$ equals 1 when $0<t-\\tau<1$. Solve this pair of inequalities for $\\tau$: subtract $t$ to get $-t<-\\tau<1-t$, then multiply by $-1$ and reverse both inequalities to get $t-1<\\tau<t$. So the integrand is $\\tau$ on the overlap of $0<\\tau<2$ with $t-1<\\tau<t$, and zero elsewhere.<br>The moving window has edges $t-1$ and $t$, and the fixed ramp has edges $0$ and $2$. Set each moving edge equal to each fixed edge: $t-1=0$, $t-1=2$, $t=0$, $t=2$. This gives $t=0,1,2,3$, which divides the calculation into five cases. In every case the antiderivative of $\\tau$ is $\\tfrac12\\tau^{2}$.<br><br><b>$t<0$:</b> the window lies left of the ramp. There is no overlap, so $y=0$.<br><b>$0<t<1$:</b> the left edge $t-1$ is below 0, so the overlap is $0<\\tau<t$: $$y(t)=\\int_{0}^{t}\\tau\\,\\d\\tau=\\Bigl[\\tfrac12\\tau^{2}\\Bigr]_{0}^{t}=\\tfrac12t^{2}-0=\\tfrac12 t^{2}.$$ <b>$1<t<2$:</b> the whole window lies inside the ramp, so the overlap is $t-1<\\tau<t$: $$\\begin{aligned}y(t)&=\\int_{t-1}^{t}\\tau\\,\\d\\tau=\\Bigl[\\tfrac12\\tau^{2}\\Bigr]_{t-1}^{t}\\\\&=\\tfrac12t^{2}-\\tfrac12(t-1)^{2}\\\\&=\\tfrac12t^{2}-\\tfrac12\\bigl(t^{2}-2t+1\\bigr)\\\\&=t-\\tfrac12.\\end{aligned}$$ <b>$2<t<3$:</b> the right edge $t$ is past 2, so the overlap is $t-1<\\tau<2$: $$\\begin{aligned}y(t)&=\\int_{t-1}^{2}\\tau\\,\\d\\tau=\\Bigl[\\tfrac12\\tau^{2}\\Bigr]_{t-1}^{2}\\\\&=\\tfrac12\\cdot4-\\tfrac12(t-1)^{2}\\\\&=2-\\tfrac12\\bigl(t^{2}-2t+1\\bigr)\\\\&=-\\tfrac12 t^{2}+t+\\tfrac32.\\end{aligned}$$ <b>$t>3$:</b> the window lies right of the ramp. There is no overlap, so $y=0$.'],
 ['Check','Three checks. <b>Continuity:</b> at $t=1$, $\\tfrac12\\cdot1^{2}=\\tfrac12$ and $1-\\tfrac12=\\tfrac12$. At $t=2$, $2-\\tfrac12=\\tfrac32$ and $-\\tfrac12\\cdot4+2+\\tfrac32=\\tfrac32$. At $t=3$, $-\\tfrac12\\cdot9+3+\\tfrac32=0$. Every pair of branches agrees. <b>Support:</b> $[0,1]$ and $[0,2]$ give $[0,3]$, because supports add. <b>Area:</b> $\\int y=\\bigl(\\int x\\bigr)\\bigl(\\int h\\bigr)=1\\times2=2$. Integrate each branch: $$\\begin{aligned}\\int_{0}^{1}\\tfrac12t^{2}\\,\\d t&=\\Bigl[\\tfrac16t^{3}\\Bigr]_{0}^{1}=\\tfrac16,\\\\\\int_{1}^{2}\\bigl(t-\\tfrac12\\bigr)\\,\\d t&=\\Bigl[\\tfrac12t^{2}-\\tfrac12t\\Bigr]_{1}^{2}=(2-1)-\\bigl(\\tfrac12-\\tfrac12\\bigr)=1,\\\\\\int_{2}^{3}\\bigl(-\\tfrac12t^{2}+t+\\tfrac32\\bigr)\\,\\d t&=\\Bigl[-\\tfrac16t^{3}+\\tfrac12t^{2}+\\tfrac32t\\Bigr]_{2}^{3}\\\\&=\\bigl(-\\tfrac{27}{6}+\\tfrac92+\\tfrac92\\bigr)-\\bigl(-\\tfrac86+2+3\\bigr)=\\tfrac92-\\tfrac{11}{3}=\\tfrac56.\\end{aligned}$$ The total is $\\tfrac16+1+\\tfrac56=2$, as required.']
]},
...
{t:'p', text:'Commutativity was derived in the two sections above by the index change $m=n-k$. The distributive property follows because a sum of two sequences can be split term by term:'},
{t:'eq', tex:'\\begin{aligned}x*(h_1+h_2)\\,[n]&=\\sum_{k=-\\infty}^{\\infty}x[k]\\bigl(h_1[n-k]+h_2[n-k]\\bigr)\\\\&=\\sum_{k=-\\infty}^{\\infty}x[k]\\,h_1[n-k]+\\sum_{k=-\\infty}^{\\infty}x[k]\\,h_2[n-k]\\\\&=(x*h_1)[n]+(x*h_2)[n].\\end{aligned}'},
...
{t:'p', text:'The second line moves $x[k]$ outside the sum over $m$, because $x[k]$ does not depend on $m$. The fourth line recognises the inner sum as the convolution $h_1*h_2$ evaluated at $n-k$. Exchanging the two sums is allowed when the double sum converges absolutely, which holds for finite-length or absolutely summable sequences. The same steps with integrals in place of sums give the continuous-time result.'},
...
{t:'p', text:'The memoryless row uses the sifting property. Substitute $h[n-k]=a\\,\\delta[n-k]$ into the convolution sum. The impulse keeps only the term $k=n$:'},
{t:'eq', tex:'y[n]=\\sum_{k=-\\infty}^{\\infty}x[k]\\,a\\,\\delta[n-k]=a\\sum_{k=-\\infty}^{\\infty}x[k]\\,\\delta[n-k]=a\\,x[n].'},
{t:'p', text:'The causal row uses the second form of the sum, $y[n]=\\sum_k h[k]\\,x[n-k]$. If $h[k]=0$ for $k<0$, every term with a negative $k$ vanishes and the sum starts at $k=0$:'},
{t:'eq', tex:'y[n]=\\sum_{k=-\\infty}^{\\infty}h[k]\\,x[n-k]=\\sum_{k=0}^{\\infty}h[k]\\,x[n-k]=h[0]x[n]+h[1]x[n-1]+h[2]x[n-2]+\\cdots'},
{t:'p', text:'Every input index $n-k$ with $k\\ge0$ satisfies $n-k\\le n$, so the output uses only present and past inputs.'},
{t:'p', text:'To show why absolute summability is sufficient for stability, assume $|x[n]|\\le B$ for all $n$. Take the modulus of the convolution sum, apply the triangle inequality (the modulus of a sum is at most the sum of the moduli), and then use the input bound on every term:'},
...
{t:'p', text:'This proves sufficiency: an absolutely summable $h[n]$ maps every bounded input to a bounded output. The bound $B\\sum_k|h[k]|$ does not depend on $n$.'},
{t:'p', text:'For necessity, suppose instead that $\\sum_k|h[k]|$ is infinite. Choose the input $x[n]=\\operatorname{sgn}h[-n]$, where $\\operatorname{sgn}$ is $+1$ for a positive argument, $-1$ for a negative argument, and $0$ at zero. This input is bounded, because $|x[n]|\\le1$. Evaluate the output at $n=0$. Since $x[-k]=\\operatorname{sgn}h[k]$, and a number times its own sign is its modulus:'},
{t:'eq', tex:'y[0]=\\sum_{k=-\\infty}^{\\infty}h[k]\\,x[0-k]=\\sum_{k=-\\infty}^{\\infty}h[k]\\operatorname{sgn}h[k]=\\sum_{k=-\\infty}^{\\infty}\\bigl|h[k]\\bigr|\\to\\infty.'},
{t:'p', text:'A bounded input gives an unbounded output, so the system is not stable. Therefore absolute summability is also necessary.'},
{t:'figrow', items:[
 {svg:()=>{const a=ax({xr:[-2,10],yr:[-0.2,1.2],xlabel:'n',w:340,h:125,pad:{l:36,r:14,t:12,b:26},xtarget:4,ytarget:2});
  ...
  cap:'$h[n]=0.7^{\\,n}u[n]$. Here $\\sum|h|=\\sum_{n=0}^{\\infty}0.7^{\\,n}=1/(1-0.7)=1/0.3<\\infty$: stable and causal.'},
 {svg:()=>{const a=ax({xr:[-2,10],yr:[-0.2,1.2],xlabel:'n',w:340,h:125,pad:{l:36,r:14,t:12,b:26},xtarget:4,ytarget:2});
