...
Object.assign(LABS, (function(){
  ...
  const L = (() => {
    ...
    /* Simpson's rule on the window; x is smooth there, so 400 panels are
       accurate far beyond the three decimals shown. At zero width the pulse
       is the impulse itself and the average is its limit, x(t0). */
    const avg = (f,t0,e)=>{ if(e<=0) return f(t0); const n=400, a=t0-e/2, d=e/n; let s=0;
    ...
    function draw(root){
      ...
      if(dt){
        ...
        A1.note(t0,f(t0),fmt(f(t0),3),{anchor:'middle',dy:-14,color:PLOT.COL.coral,fs:14,weight:600});
      ...
      if(dt){
        ...
        const I = avg(f,t0,e);
        root.querySelector('.ro').innerHTML = `
          <div><dt>Pulse height</dt><dd>${e>0?fmt(1/e,3):'∞'}</dd></div>
        ...
        root.querySelector('.derive').innerHTML = M(`
          <div class="eq"><span class="eq-label">${e>0?'Sifting with a unit-area pulse':'Sifting with the impulse'}</span>${
            T(e>0 ? `\\int_{-\\infty}^{\\infty}x(t)\\,\\delta_\\varepsilon(t-t_0)\\,\\mathrm{d}t=\\frac{1}{${fmt(e)}}\\int_{${N(t0-e/2)}}^{${N(t0+e/2)}}x(t)\\,\\mathrm{d}t=${N(I,4)}`
                  : `\\int_{-\\infty}^{\\infty}x(t)\\,\\delta(t-t_0)\\,\\mathrm{d}t=x(${N(t0)})=${N(I,4)}`,true)}</div>`);
        root.querySelector('.aper').innerHTML = M(`
          ...
          <div class="col stack">
            ...
            <div class="ctrls one">
              ...
              <div class="ctrl"><label><span>Pulse width $\\varepsilon$</span> <span class="val" data-out="eps">2</span></label>
                <input type="range" data-v="eps" min="0" max="4" step="0.1" value="2"></div>
            </div>
            <dl class="readout ro"></dl>
            <div class="derive stack"></div>
          </div></div>`);
